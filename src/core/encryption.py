...
def check_integrity(encrypted_hex, receiver_public_key):
    ...
    keystream = generate_keystream(receiver_public_key, len(ciphertext))
    ...
    try:
        ...
        printable = sum(32 <= b <= 126 for b in decrypted)
        if printable / max(len(decrypted), 1) > 0.8:
            return True
        return False
