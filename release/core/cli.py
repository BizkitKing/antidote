...
def kp_import():
    print("\n")
    raw = input("Paste JSON keypair/public key: ").strip()
    ...
    try:
        ...
        cprint("error", "Invalid JSON format.")
        return

    # Validate structure
    required_fields = ["public_key"]
    if not isinstance(parsed, dict):
        cprint("error", "Imported data must be a JSON object.")
        return

    for field in required_fields:
        if field not in parsed:
            cprint("error", f"Missing required field: {field}")
            return

    try:
        kpk = KeypairParser(keypairs_file)
        kpk.append_keypair(parsed)
        cprint("success", "Imported successfully.")
    except Exception as e:
...
def shape(message_sender_public_key, message_receiver_public_key, message):
    ...
    integrity_of_message = check_integrity(message, message_receiver_public_key)
...
def do_etheme():
    ...
    if choice == "1":
        ...
        if not hasattr(Colour, value):
            cprint("error", "Invalid colour name.")
            return
